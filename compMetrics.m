...
delete ('pMetrics.mat' , 'uComMetrics.mat', 'pComMetrics.mat');
L=500;
Fs=20;
...
for i=1:length(pData)
    ...
    for j=2:2:8
        ...
        rvelAmp=0;
        accSumDiff=0;
        if (~isempty(pData{i,j})) %checking if there is effort 1
            tempMat=pData{i,j};
            accMat=tempMat(:,1:3);
            rvelMat=tempMat(:,4:6);
...
%                 rvelFpeak(k)=f( find(rvelAmp==max(rvelAmp)) );
        ...
        if (~isempty(pData{i,j+1})) %checking if there is effort 2
            tempMat=pData{i,j+1};
            accMat=tempMat(:,1:3);
            rvelMat=tempMat(:,4:6);
