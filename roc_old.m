function ROCout=roc_old(varargin)
...
%            The second column is the column of the tag: PD (1) and
%            H (0).
...
%        default);
%        metric - string for type of metric
%        tag - string for tpye of posture
%
% Output: if verbose = 1
...
150 0;205 0;163 0;166 0;176 0;],0,0.05,1,'NOlbl','NOtag'};
default.values(1:nu) = args;
[x threshold alpha verbose metric tag] = deal(default.values{:});
if isvector(x)
...
[JJ,J]=min(d); %find the least distance
...
%         disp('Press a key to continue'); pause
    ...
    figname=strcat('ROCfigs/ROC_fig_',metric,'_',tag,'.fig');
    saveas(nikfig,figname);
